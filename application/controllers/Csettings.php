...
class Csettings extends CI_Controller {
    ...
    public function add_new_bkash() {
        ...
        $data = array(
            ...
            $bank_coa = [
             'HeadCode'         => $headcode,
             'HeadName'         => $this->input->post('bkash_no',TRUE),
    ...
    public function bkash_debit_credit_manage_add() {
        ...
        if ($this->input->post('account_type',TRUE) == "Debit(+)") {
         ...
         $receive_by=$this->session->userdata('user_id');
        $receive_date=date('Y-m-d');
        $bkashname = $this->db->select('bkash_no')->from('bkash_add')->where('bkash_id',$this->input->post('bkash_id'))->get()->row()->bkash_no;
       $coaid = $this->db->select('HeadCode')->from('acc_coa')->where('HeadName',$bkashname)->get()->row()->HeadCode;
